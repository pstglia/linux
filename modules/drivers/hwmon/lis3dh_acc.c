...
static int lis3dh_acc_input_init(struct lis3dh_acc_status *stat)
{
	...
	/* Added by wang.junxian2@byd.com */
	/* Begin */
	set_bit(ABS_X, stat->input_dev->absbit);
	set_bit(ABS_Y, stat->input_dev->absbit);
	set_bit(ABS_Z, stat->input_dev->absbit);
	/* End */
...
printk("sdl lis3dh_acc_failed --->error=%d\n",err);
	return err;
}
