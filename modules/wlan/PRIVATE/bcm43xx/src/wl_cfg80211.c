...
wl_validate_wps_ie(char *wps_ie, s32 wps_ie_len, bool *pbc)
{
	...
	while (len >= 4) {		/* must have attr id, attr len fields */
		...
		WL_ERR((" subel=%p, subelt_id=0x%x subelt_len=%u\n",
			subel, subelt_id, subelt_len));

		if (subelt_id == WPS_ID_VERSION) {
			WL_ERR(("  attr WPS_ID_VERSION: %u\n", *subel));
		} else if (subelt_id == WPS_ID_REQ_TYPE) {
			WL_ERR(("  attr WPS_ID_REQ_TYPE: %u\n", *subel));
		} else if (subelt_id == WPS_ID_CONFIG_METHODS) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_ERR(("  attr WPS_ID_CONFIG_METHODS: %x\n", HTON16(val)));
		} else if (subelt_id == WPS_ID_DEVICE_NAME) {
			char devname[100];
			memcpy(devname, subel, subelt_len);
			devname[subelt_len] = '\0';
			WL_ERR(("  attr WPS_ID_DEVICE_NAME: %s (len %u)\n",
				devname, subelt_len));
		} else if (subelt_id == WPS_ID_DEVICE_PWD_ID) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_ERR(("  attr WPS_ID_DEVICE_PWD_ID: %u\n", HTON16(val)));
			*pbc = (HTON16(val) == DEV_PW_PUSHBUTTON) ? true : false;
		} else if (subelt_id == WPS_ID_PRIM_DEV_TYPE) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_ERR(("  attr WPS_ID_PRIM_DEV_TYPE: cat=%u \n", HTON16(val)));
			valptr[0] = *(subel + 6);
			valptr[1] = *(subel + 7);
			WL_ERR(("  attr WPS_ID_PRIM_DEV_TYPE: subcat=%u\n", HTON16(val)));
		} else if (subelt_id == WPS_ID_REQ_DEV_TYPE) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_ERR(("  attr WPS_ID_REQ_DEV_TYPE: cat=%u\n", HTON16(val)));
			valptr[0] = *(subel + 6);
			valptr[1] = *(subel + 7);
			WL_ERR(("  attr WPS_ID_REQ_DEV_TYPE: subcat=%u\n", HTON16(val)));
		} else if (subelt_id == WPS_ID_SELECTED_REGISTRAR_CONFIG_METHODS) {
			valptr[0] = *subel;
			valptr[1] = *(subel + 1);
			WL_ERR(("  attr WPS_ID_SELECTED_REGISTRAR_CONFIG_METHODS"
				": cat=%u\n", HTON16(val)));
		} else {
			WL_ERR(("  unknown attr 0x%x\n", subelt_id));
		}
...
static chanspec_t wl_cfg80211_get_shared_freq(struct wiphy *wiphy)
{
	...
	if ((err = wldev_ioctl(dev, WLC_GET_BSSID, &bssid, sizeof(bssid), false))) {
		 ...
		 */
		WL_ERR(("Not associated. Return a temp channel. \n"));
	...
	*(u32 *) wl->extra_buf = htod32(WL_EXTRA_BUF_MAX);
	if ((err = wldev_ioctl(dev, WLC_GET_BSS_INFO, wl->extra_buf,
		WL_EXTRA_BUF_MAX, false))) {
			...
			WL_ERR(("Valid BSS Found. chanspec:%d \n", chspec));
	}
	return chspec;
}
...
wl_cfg80211_del_virtual_iface(struct wiphy *wiphy, bcm_struct_cfgdev *cfgdev)
{
	...
	if (wl->p2p_supported) {
		 ...
		 */
		WL_ERR(("P2P: GO_NEG_PHASE status cleared "));
	...
	struct vif_params *params)
{
	...
	if (ap) {
		...
		if (wl->p2p_supported && wl->p2p->vif_created) {
			WL_ERR(("p2p_vif_created (%d) p2p_on (%d)\n", wl->p2p->vif_created,
			p2p_on(wl)));
		...
		WL_ERR(("Change_virtual_iface for transition from GO/AP to client/STA"));
	}
	...
	void* _net_attach)
{
	...
	if (wl->p2p_supported && wl_get_p2p_status(wl, IF_ADD)) {
		WL_ERR(("IF_ADD event called from dongle, old interface name: %s,"
	...
	if (!wl->p2p_supported || !p2p_scan(wl)) {
		...
		if (unlikely(err)) {
			if (err == BCME_EPERM)
				...
				WL_ERR((" Escan not permitted at this time (%d)\n", err));
			else
...
exit:
	if (unlikely(err)) {
		...
		if ((err == BCME_EPERM) && wl->scan_suppressed)
			WL_ERR(("Escan failed: Scan Suppressed \n"));
		else
			WL_ERR(("error (%d)\n", err));
	}
	return err;
	...
	struct cfg80211_ssid *this_ssid)
{
	...
	if (wl_get_drv_status_all(wl, REMAINING_ON_CHANNEL)) {
		WL_ERR(("Remain_on_channel bit is set, somehow it didn't get cleared\n"));
		wl_notify_escan_complete(wl, ndev, true, true);
	}
			...
			if (p2p_ssid) {
				...
				if (!wl->p2p_supported || !p2p_scan(wl)) {
					...
					if ((interworking_ie = wl_cfg80211_find_interworking_ie(
						...
						if (unlikely(err)) {
							WL_ERR(("PST DEBUG wl_cfg80211_add_iw_ie (%d)\n", err));
							goto scan_out;
						}
					...
					if (unlikely(err)) {
						WL_ERR(("PST DEBUG wl_cfgp2p_set_management_ie (%d)\n", err));
						goto scan_out;
					}
	...
	if (iscan_req) {
		...
		if (likely(!err))
			...
			WL_ERR(("PST DEBUG wl_do_iscan (%d)\n", err));
			goto scan_out;
		}
	} else if (escan_req) {
		if (wl->p2p_supported) {
			if (p2p_on(wl) && p2p_scan(wl)) {
				...
				if (unlikely(err)) {
					WL_ERR(("PST DEBUG wl_cfgp2p_enable_discovery (%d)\n", err));
					goto scan_out;
				}
			}
		}
		...
		if (likely(!err))
			...
			WL_ERR(("PST DEBUG wl_do_escan (%d)\n", err));
			goto scan_out;
		}
...
#endif /* WL_CFG80211_P2P_DEV_IF */
{
	...
	if (unlikely(err)) {
		if ((err == BCME_EPERM) && wl->scan_suppressed)
			WL_ERR(("scan not permitted at this time (%d)\n", err));
		else
			WL_ERR(("scan error (%d)\n", err));
		return err;
	}
...
#endif
	WL_ERR(("setting wpa_auth to 0x%0x\n", val));
...
#endif
			WL_ERR((" NO, is_wps_conn, Set pval | gval to WSEC"));
			wsec_val = pval | gval;

			WL_ERR((" Set WSEC to fW 0x%x \n", wsec_val));
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (sme->key_len) {
		sec = wl_read_prof(wl, dev, WL_PROF_SEC);
		WL_ERR(("wpa_versions 0x%x cipher_pairwise 0x%x\n",
...
#endif
		{
			...
			WL_ERR(("key length (%d) key index (%d) algo (%d)\n",
			...
			if (sec->auth_type == NL80211_AUTHTYPE_SHARED_KEY) {
				WL_ERR(("set auth_type to shared key\n"));
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (!wl_get_drv_status(wl, CONNECTED, dev)&&
		...
		if (!ETHER_ISNULLADDR(&bssid)) {
			...
			WL_ERR(("drv status CONNECTED is not set, but connected in FW!" MACDBG "/n",
				MAC2STRDBG(bssid.octet)));
			err = wldev_ioctl(dev, WLC_DISASSOC, &scbval,
				sizeof(scb_val_t), true);
			if (unlikely(err)) {
			...
			while (wl_get_drv_status(wl, DISCONNECTING, dev) && wait_cnt) {
				WL_ERR(("Waiting for disconnection terminated, wait_cnt: %d\n",
					wait_cnt));
				wait_cnt--;
				OSL_SLEEP(10);
			}
		} else
			WL_ERR(("Currently not associated!\n"));
	} else {
	...
	if (p2p_is_on(wl) && (dev != wl_to_prmry_ndev(wl))) {
		...
		if ((wpa2_ie = bcm_parse_tlvs((u8 *)sme->ie, sme->ie_len,
			...
			WL_ERR((" WPA2 IE is found\n"));
		}
		/* find the WPA_IE */
		if ((wpa_ie = wl_cfgp2p_find_wpaie((u8 *)sme->ie,
			sme->ie_len)) != NULL) {
			WL_ERR((" WPA IE is found\n"));
		}
	...
	if (chan) {
		wl->channel = ieee80211_frequency_to_channel(chan->center_freq);
		chan_cnt = 1;
		WL_ERR(("channel (%d), center_req (%d), %d channels\n", wl->channel,
	...
	*/
	...
	if (err) {
		...
		if (err == BCME_UNSUPPORTED) {
			WL_ERR(("join iovar is not supported\n"));
			goto set_ssid;
		} else
			WL_ERR(("error (%d)\n", err));
	} else
	...
	u8 key_idx, const u8 *mac_addr, struct key_params *params)
{
	...
	if (key.len == 0) {
		...
		WL_ERR(("Setting the key index %d\n", key.index));
		...
		case WLAN_CIPHER_SUITE_WEP40:
			key.algo = CRYPTO_ALGO_WEP1;
			WL_ERR(("WLAN_CIPHER_SUITE_WEP40\n"));
			break;
		case WLAN_CIPHER_SUITE_WEP104:
			key.algo = CRYPTO_ALGO_WEP128;
			WL_ERR(("WLAN_CIPHER_SUITE_WEP104\n"));
			break;
		case WLAN_CIPHER_SUITE_TKIP:
			key.algo = CRYPTO_ALGO_TKIP;
			WL_ERR(("WLAN_CIPHER_SUITE_TKIP\n"));
			break;
		case WLAN_CIPHER_SUITE_AES_CMAC:
			key.algo = CRYPTO_ALGO_AES_CCM;
			WL_ERR(("WLAN_CIPHER_SUITE_AES_CMAC\n"));
			break;
		case WLAN_CIPHER_SUITE_CCMP:
			key.algo = CRYPTO_ALGO_AES_CCM;
			WL_ERR(("WLAN_CIPHER_SUITE_CCMP\n"));
			break;
#ifdef BCMWAPI_WPI
		case WLAN_CIPHER_SUITE_SMS4:
			key.algo = CRYPTO_ALGO_SMS4;
	...
	bool enabled, s32 timeout)
{
	...
	WL_ERR(("%s: Enter power save enabled %d\n", dev->name, enabled));
	...
	struct cfg80211_pmksa *pmksa)
{
	...
	WL_ERR(("set_pmksa,IW_PMKSA_ADD - PMKID: %pM =\n",
	...
	struct cfg80211_pmksa *pmksa)
{
	...
	WL_ERR(("del_pmksa,IW_PMKSA_REMOVE - PMKID: %pM =\n",
		&pmkid.pmkid[0].BSSID));
	for (i = 0; i < WPA2_PMKID_LEN; i++) {
...
#endif /* WL_CFG80211_P2P_DEV_IF */
{
	...
	*cookie = id;
	...
	if (wl_get_drv_status(wl, SCANNING, ndev)) {
		...
		WL_ERR(("scan is running. go to fake listen state\n"));
...
#if defined(WL_CFG80211_P2P_DEV_IF)
	if (cfgdev->iftype == NL80211_IFTYPE_P2P_DEVICE) {
		WL_ERR((" enter ) on P2P dedicated discover interface\n"));
	}
#else
	...
	struct p2p_config_af_params *config_af_params)
{
	...
	case P2P_PAF_GON_REQ: {
		WL_ERR(("P2P: GO_NEG_PHASE status set \n"));
	...
	default:
		WL_ERR(("Unknown p2p pub act frame subtype: %d\n",
			act_frm->subtype));
		err = BCME_BADARG;
	}
	return err;
	...
	void *frame, u16 frame_len)
{
	...
	WL_ERR(("result=%s", result?"true":"false"));
	return result;
}
#endif /* WL11U */
	...
	if (category == DOT11_ACTION_CAT_PUBLIC) {
		if ((action == P2P_PUB_AF_ACTION) &&
			...
			if (action == P2PSD_ACTION_ID_GAS_IREQ ||
				...
				WL_ERR(("Unknown action type: %d\n", action));
			}
		} else {
			WL_ERR(("Unknown Frame: category 0x%x, action 0x%x, length %d\n",
...
exit:
	...
	if (ack && config_af_params.extra_listen &&
		...
		if (extar_listen_time > 50) {
			...
			WL_ERR(("Wait more time! actual af time:%d,"
 ...
 */
	...
	WL_ERR(("channel from upper layer %d\n", wl->afx_hdl->peer_listen_chan));
	...
	bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	if ((len -= (WPA_IE_SUITE_COUNT_LEN + (WPA_SUITE_LEN * suite_count))) >= RSN_CAP_LEN) {
		...
		WL_ERR(("P:rsn_cap[0]=[0x%X]:wme_bss_disabled[%d]\n", rsn_cap[0], wme_bss_disable));
	...
	s32 bssidx)
{
	...
	if (dev_role == NL80211_IFTYPE_P2P_GO && (ies->wpa2_ie)) {
		...
		WL_ERR(("P2P GO: validating wpa2_ie"));
		if (wl_validate_wpa2ie(dev, ies->wpa2_ie, bssidx)  < 0)
			return BCME_ERROR;

	} else if (dev_role == NL80211_IFTYPE_AP) {

		WL_ERR(("SoftAP: validating security"));
	...
	u32 dev_role, s32 bssidx)
{
	...
	if (dev_role == NL80211_IFTYPE_P2P_GO) {
		...
		if ((err = wldev_ioctl(dev, WLC_SET_SSID, &join_params,
			...
			WL_ERR(("SoftAP set SSID (%s) success\n", join_params.ssid.SSID));
	...
	s32 bssidx)
{
	...
	if ((err = wl_cfgp2p_set_management_ie(wl, dev, bssidx,
		...
		WL_ERR(("Applied Vndr IEs for Beacon \n"));
	}
	...
	if ((err = wl_cfgp2p_set_management_ie(wl, dev, bssidx,
		...
		WL_ERR(("Applied Vndr IEs for Probe Resp \n"));
	}
	...
	s32 bssidx)
{
	...
	if (ies->wps_ie) {
		if (wl->ap_info->wps_ie &&
			...
			WL_ERR((" WPS IE is changed\n"));
			kfree(wl->ap_info->wps_ie);
			wl->ap_info->wps_ie = kmemdup(ies->wps_ie, ies->wps_ie_len, GFP_KERNEL);
		} else if (wl->ap_info->wps_ie == NULL) {
			WL_ERR((" WPS IE is added\n"));
			wl->ap_info->wps_ie = kmemdup(ies->wps_ie, ies->wps_ie_len, GFP_KERNEL);
		}
	...
	u8* mac_addr)
{
	...
	WL_ERR(("Entry\n"));
	if (mac_addr == NULL) {
		WL_ERR(("mac_addr is NULL ignore it\n"));
		return 0;
	}
	...
	struct cfg80211_ap_settings *info)
{
	...
	WL_ERR(("Enter \n"));
	if (dev == wl_to_prmry_ndev(wl)) {
		WL_ERR(("Start AP req on primary iface: Softap\n"));
		dev_role = NL80211_IFTYPE_AP;
	}
#if defined(WL_ENABLE_P2P_IF)
	else if (dev == wl->p2p_net) {
		/* Group Add request on p2p0 */
		WL_ERR(("Start AP req on P2P iface: GO\n"));
		dev = wl_to_prmry_ndev(wl);
	...
	if (p2p_is_on(wl) &&
		...
		dev_role = NL80211_IFTYPE_P2P_GO;
		WL_ERR(("Start AP req on P2P connection iface\n"));
	}
	...
	struct beacon_parameters *info)
{
	...
	bcm_tlv_t *ssid_ie;
	bool pbc = 0;
	WL_ERR(("interval (%d) dtim_period (%d) head_len (%d) tail_len (%d)\n",
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0)) && \
	(defined(WL_IFACE_COMB_NUM_CHANNELS) || defined(WL_CFG80211_P2P_DEV_IF))
	WL_ERR(("Setting interface combinations for common mode\n"));
...
/*	wdev->wiphy->flags |= WIPHY_FLAG_SUPPORTS_FW_ROAM; */
	...
	WL_ERR(("Registering custom regulatory)\n"));
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	WL_ERR(("SSID : \"%s\", rssi %d, channel %d, capability : 0x04%x, bssid %pM"
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_JOIN) {
		WL_ERR(("joined in IBSS network\n"));
	}
	if (event == WLC_E_START) {
		WL_ERR(("started IBSS network\n"));
	}
	if (event == WLC_E_JOIN || event == WLC_E_START ||
		...
		if (wl_get_drv_status(wl, CONNECTED, ndev)) {
			...
			if (memcmp(cur_bssid, &e->addr, ETHER_ADDR_LEN) == 0) {
				WL_ERR(("IBSS connected event from same BSSID("
		...
		WL_ERR(("no action - join fail (IBSS mode)\n"));
	}
	else {
		WL_ERR(("no action (IBSS mode)\n"));
	}
	return err;
}
        ...
        size_t *join_params_size)
{
	...
	if (ch != 0) {
		...
		*join_params_size += WL_ASSOC_PARAMS_FIXED_SIZE +
		...
		WL_ERR(("join_params->params.chanspec_list[0]= %X, %d channels\n",
			join_params->params.chanspec_list[0],
			join_params->params.chanspec_num));
	}
}
...
static s32 wl_update_bss_info(struct wl_priv *wl, struct net_device *ndev)
{
	...
	if (!bss) {
		...
		*(u32 *) wl->extra_buf = htod32(WL_EXTRA_BUF_MAX);
		...
		WL_ERR(("Found the AP in the list - BSSID %pM\n", bss->bssid));
#if defined(WL_CFG80211_P2P_DEV_IF)
		ie = (u8 *)bss->ies->data;
		ie_len = bss->ies->len;
#else
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (unlikely(channel_inform.scan_channel)) {
		...
		WL_ERR(("channel_inform.scan_channel (%d)\n",
...
wl_stop_wait_next_action_frame(struct wl_priv *wl)
{
	if (wl_get_drv_status_all(wl, SENDING_ACT_FRM)) {
		...
		WL_ERR(("*** Wake UP ** abort actframe iovar\n"));
		...
		WL_ERR(("*** Wake UP ** abort listen for next af frame\n"));
...

{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		if (wl_cfgp2p_is_pub_action(&mgmt_frame[DOT11_MGMT_HDR_LEN],
			...
			if (sd_act_frm && wl_get_drv_status_all(wl, WAITING_NEXT_ACT_FRM)) {
				if (wl->next_af_subtype == sd_act_frm->action) {
					WL_ERR(("We got a right next frame of SD!(%d)\n",
						sd_act_frm->action));
			...
			if (wl->next_af_subtype != P2P_PAF_SUBTYPE_INVALID) {
				...
				if (wl_get_public_action(&mgmt_frame[DOT11_MGMT_HDR_LEN],
					...
					WL_ERR(("Recived action is not public action frame\n"));
				} else if (wl->next_af_subtype == action) {
					WL_ERR(("Recived action is the waiting action(%d)\n",
						action));
					wl_clr_drv_status(wl, WAITING_NEXT_ACT_FRM, ndev);
		...
		if (act_frm) {
			...
			if (wl_get_drv_status_all(wl, WAITING_NEXT_ACT_FRM)) {
				if (wl->next_af_subtype == act_frm->subtype) {
					WL_ERR(("We got a right next frame!(%d)\n",
						act_frm->subtype));
					wl_clr_drv_status(wl, WAITING_NEXT_ACT_FRM, ndev);
		...
		if (event == WLC_E_P2P_PROBREQ_MSG) {
			...
			if (wl->p2p &&
				...
				WL_ERR(("Filtering P2P probe_req while "
	...
	WL_ERR(("mgmt_frame_len (%d) , e->datalen (%d), channel (%d), freq (%d)\n",
	...
	struct wl_scan_results **bss_list)
{
	...
	WL_ERR(("results->count = %d\n", results->count));
	WL_ERR(("results->buflen = %d\n", results->buflen));
	...
	const wl_event_msg_t *e, void *data)
{
	...
	WL_ERR((" enter event type : %d, status : %d \n",
		ntoh32(e->event_type), ntoh32(e->status)));
				...
				if (!bcmp(&bi->BSSID, &bss->BSSID, ETHER_ADDR_LEN) &&
					...
					*/
					...
					if (dtoh32(bss->length) != bi_length) {
						...
						WL_ERR(("%s("MACDBG"), replacement!(%d -> %d)\n",
						bss->SSID, MAC2STRDBG(bi->BSSID.octet),
						prev_len, bi_length));
	...
	enum wl_status state, bool set)
{
	...
	struct net_info *iter, *next;
	struct net_device *primary_dev = wl_to_prmry_ndev(wl);
	WL_ERR(("Enter state %d set %d _net_info->pm_restore %d iface %s\n",
	...
	if (set) {
		...
		for_each_ndev(wl, iter, next) {
			if (iter->pm_restore && iter->pm) {
				WL_ERR(("%s:restoring power save %s\n",
...
#if (WL_ERR_LEVEL > 0)
	s8 *estr = (event_type <= sizeof(wl_dbg_estr) / WL_ERR_ESTR_MAX - 1) ?
	    wl_dbg_estr[event_type] : (s8 *) "Unknown";
	WL_ERR(("event_type (%d):" "WLC_E_" "%s\n", event_type, estr));
#endif /* (WL_ERR_LEVEL > 0) */
            ...
            uint8 ie_id, uint8 *data, uint8 data_len)
{
	...
	if (ie_id != DOT11_MNG_INTERWORKING_ID) {
		WL_ERR(("PST DEBUG 2 wl_cfg80211_add_iw_ie ie_id = (%d)\n", ie_id));
	...
	if (wl->iw_ie_len != 0) {
		WL_ERR(("Different IW_IE was already set. clear first\n"));
		...
		if (err != BCME_OK) {
			WL_ERR(("PST DEBUG 2 wldev_iovar_setbuf_bsscfg (%d)\n", err));
			goto exit;
		}
	}
	...
	if (err == BCME_OK) {
		...
		WL_ERR(("PST DEBUG 2 wldev_iovar_setint_bsscfg (%d)\n", err));
	}
...
int wl_cfg80211_scan_suppress(struct net_device *dev, int suppress)
{
	...
	if (suppress == wl->scan_suppressed) {
		WL_ERR(("No change in scan_suppress state. Ignoring cmd..\n"));
		return 0;
	}
	...
	if (wl->scan_suppressed) {
		if (ret) {
			...
			WL_ERR(("Start wlan_timer to clear of scan_suppress \n"));
			mod_timer(&wl->scan_supp_timer,
