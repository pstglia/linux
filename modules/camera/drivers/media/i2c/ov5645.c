...
static int
ov5645_write_reg(struct i2c_client *client, u16 data_length, u16 reg, u32 val)
{
	int num_msg;
	struct i2c_msg msg;
...
static int ov5645_i2c_write(struct i2c_client *client, u16 len, u8 *data)
{
	struct i2c_msg msg;
	int ret;
	int retry = 0;
...
/*
 * __ov5645_flush_reg_array() is internal function to make writing reg
 * faster and should be not used anywhere else.
 */
static int __ov5645_flush_reg_array(struct i2c_client *client,
				     struct ov5645_write_ctrl *ctrl)
{
	u16 size;
	...
	return ov5645_i2c_write(client, size, (u8 *)&ctrl->buffer);
}

/*
 * ov5645_write_reg_array - Initializes a list of MT9T111 registers
 ...
 */
static int ov5645_write_reg_array(struct i2c_client *client,
			    const struct misensor_reg *reglist)
{
	...
	for (; next->length != MISENSOR_TOK_TERM; next++) {
		if (next->length == MISENSOR_TOK_DELAY) {
			...
			err = ov5645_write_reg(client, next->length, next->reg,
						next->val);
...
static int ov5645_af_init(struct v4l2_subdev *sd)
{
	...
	group_length = (firmware->size) / (OV5645_MAX_WRITE_BUF_SIZE - 2);
	for (i = 0; i < group_length; i++) {
		ctrl.buffer.addr = OV5645_REG_FW_START
				    + i * (OV5645_MAX_WRITE_BUF_SIZE - 2);
		memcpy(ctrl.buffer.data,
			&firmware->data[i * (OV5645_MAX_WRITE_BUF_SIZE - 2)],
			(OV5645_MAX_WRITE_BUF_SIZE - 2));
		ctrl.index = (OV5645_MAX_WRITE_BUF_SIZE - 2);
		err = __ov5645_flush_reg_array(client, &ctrl);
		if (err) {
	...
	memcpy(ctrl.buffer.data,
		&firmware->data[i * (OV5645_MAX_WRITE_BUF_SIZE - 2)],
		firmware->size - i * (OV5645_MAX_WRITE_BUF_SIZE - 2));
	ctrl.index = firmware->size - i * (OV5645_MAX_WRITE_BUF_SIZE - 2);
	err = __ov5645_flush_reg_array(client, &ctrl);
	if (err) {
		dev_err(&client->dev, "write firmwares reg failed\n");
		return err;
...
static int ov5645_s_single_focus(struct v4l2_subdev *sd, s32 value)
{
	return ov5645_s_focus_mode(sd, V4L2_CID_AUTO_FOCUS_START);
}

static int ov5645_s_cont_focus(struct v4l2_subdev *sd, s32 value)
{
	return ov5645_s_focus_mode(sd, V4L2_CID_FOCUS_AUTO);
}
...
static int ov5645_pause_focus(struct v4l2_subdev *sd, s32 value)
{
	...
	return ov5645_s_focus_mode(sd, V4L2_CID_3A_LOCK);
}

static int ov5645_release_focus(struct v4l2_subdev *sd, s32 value)
{
	return ov5645_s_focus_mode(sd, V4L2_CID_AUTO_FOCUS_STOP);
}

static int ov5645_s_color_effect(struct v4l2_subdev *sd, int effect)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov5645_device *dev = to_ov5645_sensor(sd);
	int err = 0;
	...
	case V4L2_COLORFX_NONE:
		err = ov5645_write_reg_array(client, ov5645_normal_effect);
		break;
	case V4L2_COLORFX_SEPIA:
		err = ov5645_write_reg_array(client, ov5645_sepia_effect);
		break;
	case V4L2_COLORFX_NEGATIVE:
		err = ov5645_write_reg_array(client, ov5645_negative_effect);
		break;
	case V4L2_COLORFX_BW:
		err = ov5645_write_reg_array(client, ov5645_bw_effect);
		break;
	case V4L2_COLORFX_SKY_BLUE:
		err = ov5645_write_reg_array(client, ov5645_blue_effect);
		break;
	case V4L2_COLORFX_GRASS_GREEN:
		err = ov5645_write_reg_array(client, ov5645_green_effect);
		break;
	default:
...
static int ov5645_g_image_brightness(struct v4l2_subdev *sd, int *brightness)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	/* get target image luminance average */
	return ov5645_read_reg(client, MISENSOR_8BIT,
					OV5645_REG_AE_AVERAGE, brightness);
}

static int ov5645_g_focus_status(struct v4l2_subdev *sd, int *status)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;
	u32 val = 0;
...
/* calculate sysclk */
static int ov5645_get_sysclk(struct v4l2_subdev *sd, unsigned int *sysclk)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;
	u32 temp1 = 0, temp2 = 0;
	...
	err = ov5645_read_reg(client, MISENSOR_8BIT,
					OV5645_REG_PLL_CTRL_2, &temp1);
	if (err)
		return err;
	multiplier = temp1;

	err = ov5645_read_reg(client, MISENSOR_8BIT,
					OV5645_REG_PLL_CTRL_3, &temp1);
	if (err)
		return err;
	prediv = temp1 & 0x0f;
	...
	vco = OV5645_XVCLK * multiplier / prediv;
...
static int ov5645_get_hts(struct v4l2_subdev *sd, unsigned int *hts)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;

	err = ov5645_read_reg(client, MISENSOR_16BIT,
				OV5645_REG_TIMING_HTS, hts);
	if (err)
		return err;
...
static int ov5645_get_vts(struct v4l2_subdev *sd, unsigned int *vts)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;

	err = ov5645_read_reg(client, MISENSOR_16BIT,
				OV5645_REG_TIMING_VTS, vts);
	if (err)
		return err;
...
static int ov5645_set_vts(struct v4l2_subdev *sd, unsigned int vts)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	return ov5645_write_reg(client, MISENSOR_16BIT,
					OV5645_REG_TIMING_VTS, vts);
}

/* read shutter, in number of line period */
static int ov5645_get_shutter(struct v4l2_subdev *sd, unsigned int *shutter)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;
	u32 val, temp;
	...
	err = ov5645_read_reg(client, MISENSOR_16BIT,
					OV5645_REG_EXPOSURE_0, &val);
	if (err)
		return err;
	temp = (val & 0x0fff);

	err = ov5645_read_reg(client, MISENSOR_8BIT,
					OV5645_REG_EXPOSURE_1, &val);
	if (err)
		return err;
...
static int ov5645_set_shutter(struct v4l2_subdev *sd, unsigned int shutter)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;
	u32 temp;
	...
	temp = temp << 4;
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_EXPOSURE_1, temp);
	if (err)
		return err;

	temp = shutter >> 4;
	return ov5645_write_reg(client, MISENSOR_16BIT,
					OV5645_REG_EXPOSURE_0, temp);
}

/* read gain, 16 = 1x */
static int ov5645_get_gain16(struct v4l2_subdev *sd, unsigned int *gain16)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;
	u32 val;
...
/* write gain, 16 = 1x */
static int ov5645_set_gain16(struct v4l2_subdev *sd, unsigned int gain16)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
 ...
 * adjust EV value.
 */
static int ov5645_get_exposure_bias(struct v4l2_subdev *sd, s32 *value)
{
	*value = 0;
 ...
 */
static int ov5645_get_iso(struct v4l2_subdev *sd, s32 *value)
{
	u32 gain;
	int err;

	err = ov5645_get_gain16(sd, &gain);
	if (err)
		return err;
...
static int ov5645_get_light_frequency(struct v4l2_subdev *sd,
				unsigned int *light_frequency)
{
	...
	if (temp & OV5645_AUTO_BAND) {
		/* manual */
		err = ov5645_read_reg(client, MISENSOR_8BIT,
					OV5645_REG_LIGHT_CTRL_1, &temp);
		if (err)
			return err;
		if (temp & 0x04)
			/* 50Hz */
			*light_frequency = OV5645_LIGHT_50HZ;
		else
			/* 60Hz */
			*light_frequency = OV5645_LIGHT_60HZ;
	} else {
		/* auto */
		err = ov5645_read_reg(client, MISENSOR_8BIT,
					OV5645_REG_LIGHT_CTRL_2, &temp);
		if (err)
			return err;
		if (temp & 0x01)
			/* 50Hz */
...
static int ov5645_set_bandingfilter(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov5645_device *dev = to_ov5645_sensor(sd);
	...
	/* read preview PCLK */
	err = ov5645_get_sysclk(sd, &dev->preview_sysclk);
	if (err)
		return err;

	/* read preview HTS */
	err = ov5645_get_hts(sd, &dev->preview_hts);
	if (err)
		return err;

	/* read preview VTS */
	err = ov5645_get_vts(sd, &dev->preview_vts);
	if (err)
		return err;
	...
	err = ov5645_write_reg(client, MISENSOR_16BIT, OV5645_REG_B60_STEP,
								band_step60);
	if (err)
		return err;
	...
	err = ov5645_write_reg(client, MISENSOR_16BIT, OV5645_REG_B50_STEP,
								band_step50);
	if (err)
		return err;

	max_band50 = (dev->preview_vts - 4) / band_step50;
	return ov5645_write_reg(client, MISENSOR_8BIT,
						OV5645_REG_B50_MAX, max_band50);
}
...
static int ov5645_set_ae_target(struct v4l2_subdev *sd, unsigned int target)
{
	...
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_AE_STAB_IN_H, dev->ae_high);
	if (err)
		return err;
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_AE_STAB_IN_L, dev->ae_low);
	if (err)
		return err;
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_AE_STAB_OUT_H, dev->ae_high);
	if (err)
		return err;
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_AE_STAB_OUT_L, dev->ae_low);
	if (err)
		return err;
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_AE_FAST_H, fast_high);
	if (err)
		return err;
	return ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_AE_FAST_L, fast_low);
}

static int
ov5645_set_ag_ae(struct i2c_client *client, int enable)
{
	return ov5645_write_reg(client, MISENSOR_8BIT,
			OV5645_REG_AE_MODE_CTRL,
			enable ? OV5645_AUTO_AG_AE : OV5645_MANUAL_AG_AE);
}

static int ov5645_set_night_mode(struct v4l2_subdev *sd, int enable)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int temp;
	int err;
...
static int ov5645_set_awb_gain_mode(struct v4l2_subdev *sd, int mode)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int temp;
	int err;
	...
	err = ov5645_read_reg(client, MISENSOR_8BIT,
					OV5645_REG_AWB_CTRL, &temp);
	if (err)
		return err;

	switch (mode) {
	case OV5645_AWB_GAIN_AUTO:
		/* set awb gain to auto */
		temp = temp & 0xfe;
		err = ov5645_write_reg(client, MISENSOR_8BIT,
						OV5645_REG_AWB_CTRL, temp);
		break;
	case OV5645_AWB_GAIN_MANUAL:
...
static int ov5645_stop_preview(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov5645_device *dev = to_ov5645_sensor(sd);
	int err;
	...
	err = ov5645_get_gain16(sd, &dev->preview_gain16);
	if (err)
		return err;
	...
	/* get average */
	return ov5645_read_reg(client, MISENSOR_8BIT,
				OV5645_REG_AE_AVERAGE, &dev->average);
}

static int ov5645_start_video(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err;
#if 0
	...
	err = ov5645_set_bandingfilter(sd);
	if (err)
		return err;
	...
	return ov5645_set_night_mode(sd, 0);
}

static int ov5645_start_capture(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ov5645_device *dev = to_ov5645_sensor(sd);
	...
	/* read capture VTS */
	err = ov5645_get_vts(sd, &capture_vts);
	if (err)
		return err;

	err = ov5645_get_hts(sd, &capture_hts);
	if (err)
		return err;

	err = ov5645_get_sysclk(sd, &capture_sysclk);
	if (err)
		return err;

	/* calculate capture banding filter */
	err = ov5645_get_light_frequency(sd, &light_frequency);
	if (err)
		return err;

	if (light_frequency == OV5645_LIGHT_60HZ) {
		/* 60Hz */
	...
	/* write capture gain */
	err = ov5645_set_gain16(sd, capture_gain16);
	if (err)
		return err;
	...
	if (capture_shutter > (capture_vts - 4)) {
		...
		err = ov5645_set_vts(sd, capture_vts);
		if (err)
			return err;
	}
	if (ov5645_read_reg(client, MISENSOR_8BIT,
...
static int ov5645_standby(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	return ov5645_write_reg_array(client, ov5645_standby_reg);
}

static int ov5645_wakeup(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	return ov5645_write_reg_array(client, ov5645_wakeup_reg);
}

static int __ov5645_init(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
...
static int ov5645_s_power(struct v4l2_subdev *sd, int power)
{
	if (power == 0)
...
static int ov5645_try_res(u32 *w, u32 *h)
{
	int i;
	 ...
	 */
	for (i = 0; i < N_RES; i++) {
		if (ov5645_res[i].width >= *w &&
		    ov5645_res[i].height >= *h)
			break;
	}
	...
	*w = ov5645_res[i].width;
	*h = ov5645_res[i].height;

	return 0;
}

static struct ov5645_res_struct *ov5645_to_res(u32 w, u32 h)
{
	int  index;

	for (index = 0; index < N_RES; index++) {
		if (ov5645_res[index].width == w &&
		    ov5645_res[index].height == h)
			break;
	}
	...
	return &ov5645_res[index];
}

static int ov5645_try_mbus_fmt(struct v4l2_subdev *sd,
				struct v4l2_mbus_framefmt *fmt)
{
	return ov5645_try_res(&fmt->width, &fmt->height);
}

static int ov5645_res2size(unsigned int res, int *h_size, int *v_size)
{
	...
	case OV5645_RES_QVGA:
		hsize = OV5645_RES_QVGA_SIZE_H;
		vsize = OV5645_RES_QVGA_SIZE_V;
		break;
	case OV5645_RES_DVGA:
		hsize = OV5645_RES_DVGA_SIZE_H;
		vsize = OV5645_RES_DVGA_SIZE_V;
		break;
	case OV5645_RES_320P:
		hsize = OV5645_RES_320P_SIZE_H;
		vsize = OV5645_RES_320P_SIZE_V;
		break;
	case OV5645_RES_360P:
		hsize = OV5645_RES_360P_SIZE_H;
		vsize = OV5645_RES_360P_SIZE_V;
		break;
	case OV5645_RES_VGA:
		hsize = OV5645_RES_VGA_SIZE_H;
		vsize = OV5645_RES_VGA_SIZE_V;
		break;
	case OV5645_RES_480P:
		hsize = OV5645_RES_480P_SIZE_H;
		vsize = OV5645_RES_480P_SIZE_V;
		break;
	case OV5645_RES_720P:
		hsize = OV5645_RES_720P_SIZE_H;
		vsize = OV5645_RES_720P_SIZE_V;
		break;
	case OV5645_RES_1080P:
		hsize = OV5645_RES_1080P_SIZE_H;
		vsize = OV5645_RES_1080P_SIZE_V;
		break;
	case OV5645_RES_1088P:
		hsize = OV5645_RES_1088P_SIZE_H;
		vsize = OV5645_RES_1088P_SIZE_V;
		break;
	case OV5645_RES_D3M:
		hsize = OV5645_RES_D3M_SIZE_H;
		vsize = OV5645_RES_D3M_SIZE_V;
		break;
	case OV5645_RES_3M:
		hsize = OV5645_RES_3M_SIZE_H;
		vsize = OV5645_RES_3M_SIZE_V;
		break;
	case OV5645_RES_D5M:
		hsize = OV5645_RES_D5M_SIZE_H;
		vsize = OV5645_RES_D5M_SIZE_V;
		break;
	case OV5645_RES_5M:
		hsize = OV5645_RES_5M_SIZE_H;
		vsize = OV5645_RES_5M_SIZE_V;
		break;
	default:
				...
				struct v4l2_mbus_framefmt *fmt)
{
	...
	ret = ov5645_res2size(dev->res, &width, &height);
	if (ret)
		return ret;
	fmt->width = width;
	fmt->height = height;
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	struct ov5645_device *dev = to_ov5645_sensor(sd);
	struct ov5645_res_struct *res_index;
	u32 width = fmt->width;
	...
	case OV5645_RES_QVGA:
		...
		break;
	case OV5645_RES_DVGA:
		ret = ov5645_write_reg_array(c, ov5645_dvga_init);
		break;
	case OV5645_RES_320P:
		ret = ov5645_write_reg_array(c, ov5645_320p_init);
		break;
	case OV5645_RES_360P:
	...
	case OV5645_RES_480P:
		...
		break;
	case OV5645_RES_720P:
		ret = ov5645_write_reg_array(c, ov5645_720p_init);
		break;
	case OV5645_RES_1080P:
		ret = ov5645_write_reg_array(c, ov5645_1080p_init);
		break;
	case OV5645_RES_1088P:
		ret = ov5645_write_reg_array(c, ov5645_1088p_init);
		break;
	case OV5645_RES_D3M:
		ret = ov5645_write_reg_array(c, ov5645_D3M_init);
		break;
	case OV5645_RES_3M:
		ret = ov5645_write_reg_array(c, ov5645_3M_init);
		break;
	case OV5645_RES_D5M:
		ret = ov5645_write_reg_array(c, ov5645_D5M_init);
		break;
	case OV5645_RES_5M:
		ret = ov5645_write_reg_array(c, ov5645_5M_init);
		break;
	default:
	...
	if (dev->res != res_index->res) {
		 ...
		 */
		for (index = 0; index < N_RES; index++) {
			if (width == ov5645_res[index].width &&
			    height == ov5645_res[index].height) {
				ov5645_res[index].used = 1;
				continue;
			}
...
static int ov5645_detect(struct i2c_client *client,  u16 *id, u8 *revision)
{
	...
	if (ov5645_read_reg(client, MISENSOR_16BIT,
		OV5645_REG_PID, &retvalue)) {
...
static int
ov5645_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	struct ov5645_device *dev = to_ov5645_sensor(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	u8 sensor_revision;
	...
	ret = ov5645_detect(client, &sensor_id, &sensor_revision);
	if (ret) {
		dev_err(&client->dev, "ov5645_detect err s_config.\n");
		goto fail_detect;
	}
...
static int ov5645_g_focal(struct v4l2_subdev *sd, s32 *val)
{
	*val = (OV5645_FOCAL_LENGTH_NUM << 16) | OV5645_FOCAL_LENGTH_DEM;
	return 0;
}

static int ov5645_g_fnumber(struct v4l2_subdev *sd, s32 *val)
{
	/* const f number for OV5645 */
	*val = (OV5645_F_NUMBER_DEFAULT_NUM << 16) | OV5645_F_NUMBER_DEM;
	return 0;
}

static int ov5645_g_fnumber_range(struct v4l2_subdev *sd, s32 *val)
{
	*val = (OV5645_F_NUMBER_DEFAULT_NUM << 24) |
		(OV5645_F_NUMBER_DEM << 16) |
...
static struct ov5645_control ov5645_controls[] = {
	...
	},
	{
		...
		.query = ov5645_get_exposure_bias,
	},
	{
		.qc = {
	...
	},
	{
		...
		.query = ov5645_g_image_brightness,
	},
	{
		.qc = {
			.id = V4L2_CID_COLORFX,
		...
		.tweak = ov5645_s_color_effect,
		.query = ov5645_g_color_effect,
	},
	{
		.qc = {
			...
			.name = "focal length",
			.minimum = 0,
			.maximum = OV5645_FOCAL_LENGTH_DEFAULT,
			.step = 0x01,
			.default_value = OV5645_FOCAL_LENGTH_DEFAULT,
			.flags = 0,
		},
		.query = ov5645_g_focal,
	},
	{
		.qc = {
			...
			.name = "f-number",
			.minimum = 0,
			.maximum = OV5645_F_NUMBER_DEFAULT,
			.step = 0x01,
			.default_value = OV5645_F_NUMBER_DEFAULT,
			.flags = 0,
		},
		.query = ov5645_g_fnumber,
	},
	{
		.qc = {
			...
			.name = "f-number range",
			.minimum = 0,
			.maximum =  OV5645_F_NUMBER_RANGE,
			.step = 0x01,
			.default_value = OV5645_F_NUMBER_RANGE,
			.flags = 0,
		},
...
static struct ov5645_control *ov5645_find_control(__u32 id)
{
	int i;

	for (i = 0; i < N_CONTROLS; i++) {
		if (ov5645_controls[i].qc.id == id)
...
{
	struct ov5645_control *ctrl = ov5645_find_control(qc->id);
...
{
	struct ov5645_control *octrl = ov5645_find_control(ctrl->id);
	int ret;
...
static int ov5645_mipi_stream(struct i2c_client *client, int enable)
{
	return ov5645_write_reg(client, MISENSOR_8BIT,
			OV5645_REG_FRAME_CTRL,
			enable ? OV5645_FRAME_START : OV5645_FRAME_STOP);
}
...
static int ov5645_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		case CI_MODE_PREVIEW:
			...
			break;
		case CI_MODE_VIDEO:
			err = ov5645_start_video(sd);
			break;
		case CI_MODE_STILL_CAPTURE:
			err = ov5645_start_capture(sd);
			break;
		default:
			dev_err(&client->dev,
				"invalid run mode.\n");
		...
		err = ov5645_mipi_stream(client, enable);
		if (err)
			return err;
...
static int
ov5645_enum_framesizes(struct v4l2_subdev *sd, struct v4l2_frmsizeenum *fsize)
{
	...
	fsize->discrete.width = ov5645_res[index].width;
	fsize->discrete.height = ov5645_res[index].height;

	fsize->reserved[0] = ov5645_res[index].used;
				       ...
				       struct v4l2_frmivalenum *fival)
{
	...
	for (i = 0; i < N_RES; i++) {
		if (ov5645_res[i].width >= fival->width &&
		    ov5645_res[i].height >= fival->height)
			break;
	}
	if (i == N_RES)
		i--;
	...
	fival->discrete.denominator = ov5645_res[index].fps;

	return 0;
}

static int
ov5645_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	struct v4l2_subdev_frame_size_enum *fse)
{
	...
	fse->min_width = ov5645_res[index].width;
	fse->min_height = ov5645_res[index].height;
	fse->max_width = ov5645_res[index].width;
	fse->max_height = ov5645_res[index].height;

	return 0;
}

static int
ov5645_get_pad_format(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh,
		       struct v4l2_subdev_format *fmt)
{
	struct ov5645_device *snr = to_ov5645_sensor(sd);
...
static int
ov5645_set_pad_format(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh,
		       struct v4l2_subdev_format *fmt)
{
	struct ov5645_device *snr = to_ov5645_sensor(sd);
...
/* set focus zone */
static int
ov5645_set_selection(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh,
			     struct v4l2_subdev_selection *sel)
{
	...
	for (index = 0; index < N_RES; index++) {
		if (dev->res == ov5645_res[index].res) {
			width = ov5645_res[index].width;
			height = ov5645_res[index].height;
			break;
		}
	}

	focus_width_step = width / OV5645_FOCUS_ZONE_ARRAY_WIDTH;
	focus_height_step = height / OV5645_FOCUS_ZONE_ARRAY_HEIGHT;
	...
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_FOCUS_ZONE_X,
					x_center);
	if (err)
		return err;
	err = ov5645_write_reg(client, MISENSOR_8BIT,
					OV5645_REG_FOCUS_ZONE_Y,
					y_center);
	if (err)
		return err;
	err = ov5645_write_reg(client, MISENSOR_8BIT,
	...
	return ov5645_s_focus_mode(sd, dev->focus_mode);
}
...
static int
ov5645_g_skip_frames(struct v4l2_subdev *sd, u32 *frames)
{
	int index;
	struct ov5645_device *snr = to_ov5645_sensor(sd);

	for (index = 0; index < N_RES; index++) {
		if (ov5645_res[index].res == snr->res) {
			*frames = ov5645_res[index].skip_frames;
			return 0;
		}
	}
	return -EINVAL;
...
static const struct v4l2_subdev_pad_ops ov5645_pad_ops = {
	...
	.get_fmt = ov5645_get_pad_format,
	.set_fmt = ov5645_set_pad_format,
	.set_selection = ov5645_set_selection,
};
...
static const struct media_entity_operations ov5645_entity_ops;

static int ov5645_remove(struct i2c_client *client)
{
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct ov5645_device *dev = container_of(sd,
		       ...
		       const struct i2c_device_id *id)
{
	...
	dev->color_effect = -1;
	dev->preview_gain16 = OV5645_DEFAULT_GAIN;
	dev->preview_shutter = OV5645_DEFAULT_SHUTTER;
...
static __exit void ov5645_exit_mod(void)
{
	i2c_del_driver(&ov5645_driver);
}

module_init(ov5645_init_mod);
module_exit(ov5645_exit_mod);
