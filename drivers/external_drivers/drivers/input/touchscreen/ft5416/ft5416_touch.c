...
static int ft5416_touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ft5416_chip_reset();
      printk("touch_reset_pin ==%d",gpio_get_value(ft5416_touch_pdata->reset));
/* Check if the I2C bus supports BYTE transfer */
	...
	/*init INTERRUPT pin*/
	err = gpio_request(ft5416_touch_pdata->irq, 0);
	if(err < 0)
	{
		dev_err(&client->dev,
	...
	if (err)
	{
		...
		gpio_free(ft5416_touch_pdata->irq);
	      ...
	      printk("touch_irq_pin ==%d",gpio_get_value(ft5416_touch_pdata->irq));
