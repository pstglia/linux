...
#define IAP_PORTION           0  //update_firware change 
#define PAGERETRY  30
#define IAPRESTART 5
...
static int elan_iap_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
static int __elan_ktf2k_ts_poll(struct i2c_client *client)
{
	...
	printk( "\n[elan]%s: poll interrupt status %s\n",
			...
			uint8_t *buf, size_t size)
{
	...
	dev_dbg(&this_client->dev, "[elan]%s: enter\n", __func__);
	...
	if (rc < 0)
		...
		if (i2c_master_recv(this_client, buf, size) != size ||
		    buf[0] != CMD_S_PKT)
			return -EINVAL;
	}
	return 0;
...
static int elan_ktf2k_ts_setup(struct i2c_client *client)
{
	...
	rc = elan_ktf2k_ts_poll(client);
	if (rc < 0) {
		printk( "[elan] %s: Int poll failed!66_fail !!\n", __func__);
	}
	else{
		rc = i2c_master_recv(client, buf_recv, 8);
		printk("[elan] %s: 66 packet %2x:%2X:%2x:%2x:%2x:%2X:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3] , buf_recv[4], buf_recv[5], buf_recv[6], buf_recv[7]);
		if(buf_recv[0]==0x66)
		{
			...
			const struct i2c_device_id *id)
{
	...
	if (err)
	{
	   ...
	   /*init INTERRUPT pin*/
	err = gpio_request(elan_pdata->intr_gpio, 0);
	if(err < 0)
	{
