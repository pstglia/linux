...
#define S5K8AAY_CHIP_ID_VAL	0x05CA
...
struct s5k5cagx_format_struct {
	u8 *desc;
	u32 pixelformat;
	struct regval_list *regs;
};

struct s5k5cagx_res_struct {
	u8 *desc;
	int res;
	int width;
	int height;
...
struct s5k5cagx_write_buffer {
	u16 addr;
	u8 data[S5K8AAY_MAX_WRITE_BUF_SIZE];
};

struct s5k5cagx_write_ctrl {
	int index;
	struct s5k5cagx_write_buffer buffer;
};

/*
 * Modes supported by the s5k5cagx driver.
 * Please, keep them in ascending order.
 */
static struct s5k5cagx_res_struct s5k5cagx_res[] = {
