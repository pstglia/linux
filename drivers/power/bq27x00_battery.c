 ...
 * Or < 0 if something fails.
 */
static void bq27x00_battery_update_temperature(struct bq27x00_device_info *di)
{
...
static int bq27x00_battery_read_temperature(struct bq27x00_device_info *di)
{
	...
	bq27x00_battery_update_temperature(di);
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		...
		dev_info(di->dev,"%s:technology:%d\n",__func__,di->pdata->technology);
		break;
	...
	case POWER_SUPPLY_PROP_MODEL_NAME:
		if (!strncmp(di->pdata->battid, "UNKNOWNB", 8)){
			...
			dev_info(di->dev,"%s:battid_model_name_1:%s\n",__func__,di->pdata->battid);
		}else{
			val->strval = di->pdata->model_name;
			dev_info(di->dev,"%s:model_name_2:%s\n",__func__,di->pdata->model_name);
		}
		break;
