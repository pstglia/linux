...
static bool can_manipulate_uids(void)
{
	...
	return in_egroup_p(xt_qtaguid_ctrl_file->gid)
		|| unlikely(!xfs_kuid_to_uid(current_fsuid())) || unlikely(!proc_ctrl_write_limited)
		|| unlikely(xfs_kuid_to_uid(current_fsuid()) == xfs_kuid_to_uid(xt_qtaguid_ctrl_file->uid));
}

static bool can_impersonate_uid(uid_t uid)
{
	return uid == xfs_kuid_to_uid(current_fsuid()) || can_manipulate_uids();
}
...
static bool can_read_other_uid_stats(uid_t uid)
{
	...
	return in_egroup_p(xt_qtaguid_stats_file->gid)
		|| unlikely(!xfs_kuid_to_uid(current_fsuid())) || uid == xfs_kuid_to_uid(current_fsuid())
...
static void put_utd_entry(struct uid_tag_data *utd_entry)
{
	...
	if (RB_EMPTY_ROOT(&utd_entry->tag_ref_tree) &&
		...
		DR_DEBUG("qtaguid: %s(): "
			 ...
			 current->pid, current->tgid, xfs_kuid_to_uid(current_fsuid()));
...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (sk != NULL) {
		...
		MT_DEBUG("qtaguid[%d]: filp...uid=%u\n",
			par->hooknum, filp ? xfs_kuid_to_uid(filp->f_cred->fsuid) : -1);
	}
	...
	sock_uid = xfs_kuid_to_uid(filp->f_cred->fsuid);
	/*
	 * TODO: unhack how to force just accounting.
	 ...
	 */
	if (info->match & XT_QTAGUID_UID)
		if ((xfs_kuid_to_uid(filp->f_cred->fsuid) >= info->uid_min &&
		     xfs_kuid_to_uid(filp->f_cred->fsuid) <= info->uid_max) ^
	...
	if (info->match & XT_QTAGUID_GID)
		if ((xfs_kgid_to_gid(filp->f_cred->fsgid) >= info->gid_min &&
				xfs_kgid_to_gid(filp->f_cred->fsgid) <= info->gid_max) ^
...
static int ctrl_cmd_delete(const char *input)
{
	...
	if (argc < 3) {
		...
		pr_info("qtaguid: ctrl_delete(%s): "
			...
			input, current->pid, current->tgid, xfs_kuid_to_uid(current_fsuid()));
		res = -EPERM;
		goto err;
	}
...
static int ctrl_cmd_tag(const char *input)
{
	...
	CT_DEBUG("qtaguid: ctrl_tag(%s): "
		 ...
		 current_euid(), xfs_kuid_to_uid(current_fsuid()),
...
static int qtudev_open(struct inode *inode, struct file *file)
{
	...
	utd_entry = get_uid_data(xfs_kuid_to_uid(current_fsuid()), &utd_entry_found);
	if (IS_ERR_OR_NULL(utd_entry)) {
	...
	DR_DEBUG("qtaguid: tracking data for uid=%u in pqd=%p\n",
		 xfs_kuid_to_uid(current_fsuid()), new_pqd_entry);
